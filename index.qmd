...
4. **Open the notebook** in VS Code or Quarto preview
...
If we're going to run into issues, it's likely here, hence the slightly complex package loading code

```{julia}
#| output: false
import Pkg
for url in [
    "https://github.com/dossgollin-lab/ICOW.jl",
    "https://github.com/dossgollin-lab/SimOptDecisions",
]
    pkg = replace(basename(url), ".jl" => "")
    try
        Base.require(Main, Symbol(pkg))  # check if it actually loads
    catch
        Pkg.add(; url)
    end
end
...
baseline = StaticPolicy(
    a_frac=0.15, w_frac=0.0, b_frac=0.0, r_frac=0.0, P=0.0
)
```
...
Now it's your turn.
Each experiment asks you to modify one parameter and observe the effect on risk.
...
Use the city parameters you estimated on Wednesday to create a custom configuration.
Fill in your city's values for `H_city`, `D_city`, `W_city`, and `H_seawall`, then run EAD to compare against Manhattan.
